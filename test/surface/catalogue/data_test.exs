defmodule Surface.Catalogue.DataTest do
  ...
  test "pop" do
    map = %{lists: [%{id: "List_1"}, %{id: "List_2"}]}
    assert Data.pop(map.lists[& &1.id == "List_2"]) == {[%{id: "List_2"}], %{lists: [%{id: "List_1"}]}}
  end

  test "update" do
    map = %{lists: [%{value: 2}, %{value: 3}]}
    assert Data.update(map.lists[_].value, fn v -> v*v end) == %{lists: [%{value: 4}, %{value: 9}]}
  end

  test "get_and_update" do
    map = %{lists: [%{value: 2}, %{value: 3}]}
    assert Data.get_and_update(map.lists[1].value, &{&1, &1 * &1}) == {3, %{lists: [%{value: 2}, %{value: 9}]}}
  end
